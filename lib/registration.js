...
var Registration = function(dbConnection) {
    ...
    var checkIfPhoneNumberExist = function (app) {

        var sqlToCheckIfPhoneNumberExist = 'SELECT phoneNumber From users WHERE phoneNumber = ' + dbConnection.escape(app.phoneNumber);;
        dbConnection.query(sqlToCheckIfPhoneNumberExist, function (err, rows) {
            assert.ok(err === null, err);
            if(rows === undefined || rows.length === 0) {
                self.emit("phoneNumber-does-not-exist", app);
            } else {
                app.invalidate("This phoneNumber already exists");
                app.code = 10002;
                self.emit("invalid", app);
            }
    ...
    self.on("validated", checkIfPhoneNumberExist);
    self.on("phoneNumber-does-not-exist", createUser);
